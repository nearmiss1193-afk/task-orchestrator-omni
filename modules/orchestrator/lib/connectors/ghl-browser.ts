...
export class GHLBrowserConnector extends BaseConnector {
    ...
    async editFunnelStep({ funnelName, stepName, htmlContent }: any, page: Page) {
        ...
        // Assuming we are already in the funnel context from createFunnelPage
        // If not, we would need navigation logic. For this flow, we assume sequence.

        // Wait for "Edit" button to be available on the *step*
        try {
            // Sometimes GHL takes a moment to refresh the step list
            await new Promise(r => setTimeout(r, 3000));

            // Click "Edit" -> "Edit Page"
            // The structure is often: A card for the step -> Edit button
            // If we just created it, it's likely the active step

            console.log("[GHL_Edit] Clicking 'Edit' button...");
            const editClicked = await page.evaluate(() => {
                // Look for 'Edit' text in buttons or links
            ...
            if (editClicked) {
                // Wait for Dropdown "Edit Page"
                await new Promise(r => setTimeout(r, 1000));
                console.log("[GHL_Edit] Clicking 'Edit Page'...");
                ...
                if (!editPageClicked) {
                    console.log("[GHL_Edit] 'Edit Page' not found in dropdown. Trying direct button...");
                }
            } else {
                console.log("[GHL_Edit] Main 'Edit' button not found. Attempting to find 'Edit Page' directly...");
                // sometimes it's directly exposed
            }

            // 2. Wait for Builder (New Tab or Navigation)
            // GHL often opens builder in new tab. We need to catch it.
            // OR it navigates current tab.

            console.log("[GHL_Edit] Waiting for Builder to Load...");
            // We'll race: 30s timeout or specific builder selector
            // Selector: #builder-app, .builder-canvas

            try {
                await page.waitForSelector('.builder-canvas, #app, .editor-canvas', { timeout: 45000 });
            } catch (e) {
                console.log("[GHL_Edit] Warning: Builder selector timeout. Checking if it opened in new tab...");
                const pages = await page.browser().pages();
                const lastPage = pages[pages.length - 1];
                if (lastPage !== page) {
                    console.log("[GHL_Edit] Switching to new tab...");
                    page = lastPage;
                    await page.waitForSelector('.builder-canvas, #app', { timeout: 30000 });
                }
            }

            console.log("[GHL_Edit] Builder Loaded. Preparing Injection...");
            await new Promise(r => setTimeout(r, 5000)); // Settlement time

            // 3. Inject "Custom HTML" Element
            // Strategy: 
            // a. Open Elements Menu
            // b. Find "Custom HTML"
            // c. Drag it? (Hard in Puppeteer)
            // d. Click it? (Usually adds to bottom)

            // Let's try to find the "Add Element" (+) button in the builder
            const addElementClicked = await page.evaluate(() => {
                const btns = Array.from(document.querySelectorAll('button, .icon-btn'));
                // Look for plus icon or "Add element" tooltip
            ...
            if (addElementClicked) {
                console.log("[GHL_Edit] 'Add Element' menu opened.");
                await new Promise(r => setTimeout(r, 2000));
            }

            // Search for "Custom JS/HTML" in the sidebar
            // Often has text "Custom JS/HTML"
            console.log("[GHL_Edit] Selecting 'Custom JS/HTML' element...");
            ...
            if (!customHtmlClicked) {
                console.log("[GHL_Edit] Could not click 'Custom JS/HTML'. Stopping UI automation.");
                return { success: false, error: "Could not find Custom JS/HTML element" };
            }

            await new Promise(r => setTimeout(r, 3000));

            // 4. Open Code Editor
            console.log("[GHL_Edit] Opening Code Editor...");
            // Click the new element (it should be selected, but let's click the 'Open Code Editor' button in settings sidebar)
            const openEditorClicked = await page.evaluate(() => {
                const btns = Array.from(document.querySelectorAll('button'));
            ...
            if (!openEditorClicked) throw new Error("Could not find 'Open Code Editor' button");

            await new Promise(r => setTimeout(r, 2000));

            // 5. Type Code
            console.log("[GHL_Edit] Pasting Code...");
            // It's usually an Ace editor or Monaco editor.
            // Direct DOM value setting is best.

            await page.evaluate((content) => {
                // Try to find the ace editor instance
                // @ts-ignore
                if (window.ace) {
                    // @ts-ignore
                    ...
                    // Fallback: CodeMirror or simple textarea
            ...
            // 6. Save Code
            console.log("[GHL_Edit] Saving Code Block...");
            await page.evaluate(() => {
                const btns = Array.from(document.querySelectorAll('button'));
            ...
            // 7. Save Page
            console.log("[GHL_Edit] Saving Page...");
            await page.evaluate(() => {
                const btns = Array.from(document.querySelectorAll('button'));
            ...
            await new Promise(r => setTimeout(r, 5000));
            console.log("[GHL_Edit] Success!");

            return { success: true, note: "Pasted Custom HTML and Saved." };

        } catch (error: any) {
            console.error(`[GHL_Edit] Error: ${error.message}`);
            await page.screenshot({ path: 'ghl_edit_failure.png' });
