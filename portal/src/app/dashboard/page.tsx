...
export default function Dashboard() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // FETCH SOVEREIGN STATS (RLS Bypass Proxy)
                const proxyRes = await fetch('https://nearmiss1193-afk--ghl-omni-automation-sovereign-stats.modal.run');
                const sovereignData = await proxyRes.json();

                if (sovereignData && !sovereignData.error) {
                    setStats(prev => ({
                        ...prev,
                        outbound_24h: sovereignData.outbound_24h || 0,
                        pipeline_value: (sovereignData.total_leads || 0) * 1200
                    }));

                    if (sovereignData.social_logs) {
                        const socialLogs = sovereignData.social_logs.map((s: any) => ({
                            id: `social-${s.ts}`,
                            type: 'social' as const,
                            title: s.company || 'Social Broadcast',
                            subtitle: `SOCIAL: ${s.status || 'published'}`,
                            timestamp: s.ts,
                            status: s.status || 'published'
                        }));
                        setLogs(prev => [...socialLogs, ...prev].slice(0, 20));
                    }
                }
            } catch (err) {
                console.error("Sovereign Proxy Fetch Error:", err);
            }

            // Fallback / Supplementary Data Fetch (Direct Supabase)
            ...
            setLogs(prev => {
                // Filter out duplicates from social logs added above
    ...
    return (
        <div className="flex min-h-screen bg-[#050505] text-[#e0e0e0] font-sans overflow-hidden">
            ...
            <aside className="hidden lg:flex w-64 border-r border-white/5 flex-col bg-black/40 backdrop-blur-2xl">
            ...
            {/* Main */}
            <main className="flex-1 overflow-y-auto p-4 md:p-8 space-y-8 h-screen custom-scrollbar relative pb-24 lg:pb-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                        { label: 'Outbound', value: stats.outbound_24h, icon: Send, color: 'text-blue-400' },
                        { label: 'Pipeline', value: `$${Math.round(stats.pipeline_value / 1000)}k`, icon: TrendingUp, color: 'text-purple-400' },
                        ...
                        <div key={i} className="bg-white/[0.03] border border-white/5 p-4 lg:p-6 rounded-3xl group shadow-inner">
                            <div className="flex justify-between items-start">
                                <div>
                                    <p className="text-gray-500 text-[10px] font-bold uppercase tracking-wider">{item.label}</p>
                                    <h3 className="text-xl lg:text-3xl font-bold mt-1 tracking-tighter">{item.value}</h3>
                                </div>
                                <div className={`p-2 lg:p-3 rounded-2xl bg-white/5 ${item.color} hidden sm:block`}>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <section className="lg:col-span-2 bg-white/[0.02] border border-white/5 p-6 rounded-3xl flex flex-col h-[600px]">
                        ...
                        <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar pb-32">
                            {logs.map((log) => (
                                <div
                                    key={log.id}
                                    onClick={() => log.type === 'call' && setSelectedTranscript(log.content || 'No transcript.')}
                                    className={`flex items-center justify-between p-3 lg:p-4 rounded-2xl bg-white/[0.03] border border-white/5 hover:border-blue-500/30 transition-all cursor-pointer group`}
                                >
                                    <div className="flex items-center space-x-3 lg:space-x-4">
                                        <div className={`p-2 lg:p-2.5 rounded-xl bg-white/5 group-hover:scale-110 transition-transform relative flex-shrink-0`}>
                                        ...
                                        <div className="min-w-0">
                                            <div className="flex items-center space-x-2">
                                                <h4 className="text-sm font-bold text-gray-200 group-hover:text-white truncate">{log.title}</h4>
                                            </div>
                                            <p className="text-[9px] text-gray-500 font-medium uppercase tracking-widest truncate">
                                                {log.subtitle}
                                            </p>
                                        </div>
                                    </div>
