...
def send_protocol():
    ...
    try:
        ...
        # Write debug just in case
        with open("ghl_response_dump.txt", "w", encoding="utf-8") as f:
            f.write(res.text)
        ...
        if res.status_code in [200, 201]:
            ...
            # Correct Extraction Logic based on Dump
            # {"meta":{"contactId":"CRKzurFtJZzegU6vJyn6"}}
            try:
                err_json = res.json()
                contact_id = err_json.get('meta', {}).get('contactId')
                
                if contact_id:
                     print(f"✅ Recovered Owner ID from Duplicate Meta: {contact_id}")
                else:
                     # Fallback to old behavior just in case
                     contact_id = err_json.get('contact', {}).get('id')
                     if contact_id:
                         print(f"✅ Recovered Owner ID from Contact obj: {contact_id}")
            except:
                pass
                
            if not contact_id:
                 print(f"❌ 400 Error (Parse Failed): {res.text}")
                 return
