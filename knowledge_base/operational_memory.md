...
- **Stack**: Next.js 14 + Neon PostgreSQL + Drizzle ORM + Vercel
- **Custom Domain**: <https://lakelandfinds.com> ✅ (DNS via Squarespace → Vercel)
- **Backup URL**: <https://lakeland-local-prod.vercel.app>
- **Database**: Neon (PostgreSQL) — `ep-small-hall-aefqgxkp-pooler.c-2.us-east-2.aws.neon.tech`
...
| Run | Categories | GPS Points | Unique Businesses |
|-----|-----------|------------|-------------------|
| First run | 50 | 1 (downtown) | 1,089 |
| Expanded run | 150+ subcategories | 6 (Downtown, N, S, E, W, Mulberry) | **3,321** (complete) |

### Domain Strategy (Feb 10, 2026)

| Domain | Role | Status |
|--------|------|--------|
| `lakelandfinds.com` | Directory (Yelp-style) | ✅ Live |
| `1staistep.com` | Agency funnel (test) | 🔜 Next |
| `aiserviceco.com` | Main agency brand | 🔒 Protected |
| `lakeland1st.com` | Future use | Parked |
| `lakelandlocal1.com` | Redirect | Parked |

> **Key decision**: Dan uses voice-to-text — "AI Service Code" = `aiserviceco.com`

### Cold Call Sheet v2

- **3,217 rows** sorted by priority: 506 🔴 no website, 82 🟡 low reviews, 1,175 🟢 top rated
- Pre-written openers per tier → `lakeland_call_sheet.csv` on Desktop
- Generated by `scripts/generate_call_sheet_v2.py`

### Vercel Deployment Notes

- **Env vars must be set manually** — `vercel env add DATABASE_URL production` (pipe value via echo)
- `--yes` flag is NOW supported in vercel CLI v50+ (was NOT in v48)
- Build passes locally before deploying
- `DATABASE_URL` is the only required env var for build

#### ⚠️ CRITICAL: Vercel Stale Deploy Problem (Feb 10, 2026)

**Problem**: GitHub auto-deploy silently fails. Code pushed, but Vercel never builds.
**Symptom**: `git push` succeeds, site shows old version, no build error.
**Fix**: Force deploy via CLI: `npx -y vercel --prod --yes`
**Rule**: After EVERY `git push`, verify changes are visible. If not → force deploy.
**Affected projects**: AI Service Co, LakelandFinds (multiple times)

#### Vercel Projects

| Project | Domain | Repo |
|---------|--------|------|
| lakeland-local-prod | lakelandfinds.com | nearmiss1193-afk/lakeland-local-prod |
| smoke-vape-unlimited | lakelandlocal1.com (pending DNS) | nearmiss1193-afk/smoke-vape-unlimited |

### Key Scripts

| Script | Purpose |
|--------|---------|
| `scripts/scrape_google_places.py` | First-run scraper (50 categories) |
| `scripts/scrape_expanded.py` | Expanded scraper (150+ categories × 6 GPS points) |
| `scripts/seed_from_csv.py` | Seeds DB from first CSV |
| `scripts/seed_expanded.py` | Seeds DB from expanded CSV (batch inserts) |
| `scripts/generate_call_sheet.py` | Generates personalized call sheet CSV |

### Schema Updates (Feb 10)
